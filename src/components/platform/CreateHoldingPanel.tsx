...
interface CreateHoldingPanelProps {
  open: boolean;
  onClose: () => void;
...
export default function CreateHoldingPanel({
  open,
  onClose,
  onSubmit,
  editingHolding,
}: CreateHoldingPanelProps) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "No se pudo guardar el holding.");
    } finally {
      setSubmitting(false);
    }
  }
      ...
      >
        <div className="flex items-center justify-between border-b border-navy/10 px-8 py-5">
          <h2 className="font-display text-xl font-bold text-navy">
            {editingHolding ? "Editar holding" : "Nuevo holding"}
          </h2>
          <button
            onClick={onClose}
            aria-label="Cerrar"
        ...
        <form onSubmit={handleSubmit} className="flex-1 space-y-5 p-8">
          <div>
            ...
            <input
              ...
              placeholder="Ej: Acqua Fontana Holding"
          ...
          <div className="flex justify-end gap-3 border-t border-navy/10 pt-6">
            ...
            >
              ...
              {editingHolding ? "Guardar cambios" : "Crear holding"}
            </button>
          </div>
        </form>
      </aside>
